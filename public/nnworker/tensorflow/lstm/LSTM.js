...
const tnt = require('test-network-trainer');
...
class LSTM {
    ...
    async train(inData, outData, options) {
        ...
        for(let i = 0;  i < epochs; i++){
            logger("epoch :" + i);
            modelFit = await this.model.fit(inData, outData, {
                batchSize: batchSize,
                epochs: 1,
            });
            logger("Loss after epoch " + (i+1) + ": " + modelFit.history.loss[0]);
        }
        //await model.save(`file://`+__dirname);
        //logger("file save path: " + __dirname);

        // model = await tf.loadLayersModel(`file://${FILE_SAVE_PATH + '/'+ id +'/model.json'}`);
    ...
    async predict(primer, amnt){
        ...
        return tnt.decodeOutput(output, this.indexToVocab);
    }
}